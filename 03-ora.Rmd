...
grid.rect()
grid.circle(x = -0.4, y = 0.1, r = 1, default.units = "native", gp = gpar(fill = NA))
grid.circle(x = 0.4, y = 0.1, r = 1, default.units = "native", gp = gpar(fill = NA))
grid.text("Background genes", x = -1.4, y = 1.4, default.units = "native", just = c("left", "top"))
grid.text("Differentially\nexpressed genes", x = -0.85, y = 0.3, just = "left", default.units = "native")
grid.text("Genes in\nthe gene set", x = 0.25, y = 0.3, just = "left", default.units = "native")
grid.text("Differnetially expressed genes\nalso in the gene set", x = 0, y = -1.4, just = "bottom", default.units = "native")
grid.lines(c(0, 0), c(-1.1, -0.3), arrow = arrow(angle = 15, length = unit(3, "mm")), default.units = "native")
```
...
grid.text("In Total N balls", x = 0.1, y = 0.9, just = "left")
grid.text("Pick n balls", x = 0.6, y = 0.9, just = "left")
...
lt = readRDS("data/EBI_E-GEOD-101794_expression.rds")
diff_tb = lt$diff_table

gene_list = mapIds(org.Hs.eg.db, keys = rownames(diff_tb)[diff_tb$p_adjust < 0.05], keytype = "ENSEMBL", column = "ENTREZID")
gene_list = gene_list[!is.na(gene_list)]

library(org.Hs.eg.db)
universe1 = keys(org.Hs.eg.db, keytype = "ENTREZID")

pc_genes = select(org.Hs.eg.db, key = "protein-coding", keytype = "GENETYPE", column = c("ENTREZID"))[, 2]
universe2 = pc_genes

counts = lt$counts
l = apply(counts, 1, function(x) sum(x > 5) > length(x)/2)
expressed_genes = mapIds(org.Hs.eg.db, keys = rownames(counts)[l], keytype = "ENSEMBL", column = "ENTREZID")
expressed_genes = expressed_genes[!is.na(expressed_genes)]
universe3 = intersect(expressed_genes, pc_genes)

gs = toTable(org.Hs.egGO2ALLEGS)
gs = gs[gs$Ontology == "BP", ]
universe4 = unique(gs$gene_id)

# gs = get_msigdb(version = "2023.1.Hs", collection = "h.all")
# universe4 = unique(unlist(gs))
